...
def draw_slot(surface, assets, x, y, size, key_text, cost=None, is_pressed=False, t_id=None, show_ribbon=True, tier_lvl=None, icon_img=None):
    ...
    if icon_img:
        ix = x + (size // 2) - (icon_img.get_width() // 2)
        iy = y + (size // 2) - (icon_img.get_height() // 2) + y_off - 4
        surface.blit(icon_img, (ix, iy))
    elif t_id and t_id in assets.ui_tower_icons:
        icon = assets.ui_tower_icons[t_id]
        ix = x + (size // 2) - (icon.get_width() // 2)
        iy = y + (size // 2) - (icon.get_height() // 2) + y_off - 4
        surface.blit(icon, (ix, iy))

    # ¡Arreglado! Ahora el fondo oscuro solo se dibuja si hay una tecla asignada
    if key_text:
        ...
        if assets.keys_sheet:
            ...
            key_surf = assets.ui_font_small.render(key_text.upper(), False, "white")
    ...
    if show_ribbon:
        ...
        if cost is not None:
            cost_surf = assets.ui_font_small.render(f"{cost} G", False, "black")
...
def draw_action_btn(surface, assets, x, y, w, h, key_text, label, cost=None, is_pressed=False):
    ...
    if key_text:
        ...
        if assets.keys_sheet:
            ...
            key_surf = assets.ui_font_medium.render(key_text.upper(), False, "white")
            surface.blit(key_surf, (x - 3, y - 8 + y_off))

    label_surf = assets.ui_font_medium.render(label, False, "black")
...
def draw_game_over_menu(surface, assets, current_minute, game_time, lang):
    overlay = pygame.Surface((1280, 720), pygame.SRCALPHA)
    overlay.fill((50, 0, 0, 200))
    surface.blit(overlay, (0, 0))

    title_str = lang.get("text_defeat", "DEFEAT!")
    title_text = assets.ui_font_large.render(title_str, False, "red")
    surface.blit(title_text, (640 - title_text.get_width() // 2, 250))

    survived_str = lang.get("text_survived", "You survived for {mins} minutes and {secs} seconds.").format(mins=current_minute, secs=int(game_time % 60))
    info_text = assets.ui_font_medium.render(survived_str, False, "white")
    surface.blit(info_text, (640 - info_text.get_width() // 2, 320))

    exit_str = lang.get("text_close_exit", "Close the window to exit.")
    exit_text = assets.ui_font_small.render(exit_str, False, "gray")
    surface.blit(exit_text, (640 - exit_text.get_width() // 2, 400))


def draw_level_up_menu(surface, assets, level_up_options, selected_card_idx, offsetX, width_gameboard, lang):
    overlay = pygame.Surface((1280, 720), pygame.SRCALPHA)
    ...
    card_rects = []
    is_castle_select = len(level_up_options) > 0 and level_up_options[0].get("type") in ["arrow", "fireball", "lightning", "kunai", "laser", "thorns", "smite"]

    if is_castle_select:
        title_str = lang.get("text_choose_castle", "CHOOSE YOUR INITIAL TOWER AND CASTLE")
        title_text = assets.ui_font_large.render(title_str, False, "yellow")
        surface.blit(title_text, (640 - title_text.get_width() // 2, 10))
    else:
        title_str = lang.get("text_level_up", "LEVEL UP! CHOOSE A REWARD")
        title_text = assets.ui_font_large.render(title_str, False, "yellow")
    ...
    for i, card in enumerate(level_up_options):
        ...
        if assets.lvlcard:
            ...
            base_card.fill("#112233")

        c_type = card.get("type", "")
        c_id = card.get("id", c_type)
        ...
        if c_type == "upgrade_tower" or is_castle_select:
            ...
            if "goldbag" in assets.stat_icons: base_card.blit(assets.stat_icons["goldbag"], (34, 18))
        elif c_type == "gems_5":
...
def draw_pause_menu(surface, assets, mouseX, mouseY, lang):
    overlay = pygame.Surface((1280, 720), pygame.SRCALPHA)
    ...
    menu_str = lang.get("text_menu", "MENU")
    title_text = assets.ui_font_large.render(menu_str, False, "white")
    text_x = 640 - (title_text.get_width() // 2)
    text_y = 120 + 40 - (title_text.get_height() // 2) - 8
    surface.blit(title_text, (text_x, text_y))

    menu_opts_keys = ["Resume", "Restart", "Settings", "Quit"]
    pause_rects = []
    for i, opt_key in enumerate(menu_opts_keys):
        btn_y = 250 + i * 80
        btn_rect = pygame.Rect(640 - 100, btn_y, 200, 60)
        pause_rects.append((btn_rect, opt_key))
        ...
        display_text = lang.get(f"btn_{opt_key.lower()}", opt_key)
        opt_surf = assets.ui_font_medium.render(display_text, False, "black")
        surface.blit(opt_surf, (640 - opt_surf.get_width() // 2, btn_y + 30 - opt_surf.get_height() // 2 - 4 + y_off))

    return pause_rects


def draw_main_menu(surface, assets, mouseX, mouseY, offset_y, lang):
    from gamedata import GAME_VERSION
    ...
    title_str = lang.get("text_title", "CASTLE SURVIVORS")
    title_text = assets.ui_font_large.render(title_str, False, (255, 235, 0))

    text_x = 640 - (title_text.get_width() // 2)
    text_y = 100 + offset_y + 50 - (title_text.get_height() // 2) - 8
    surface.blit(title_text, (text_x, text_y))

    menu_opts_keys = ["Play", "Upgrades", "Settings", "Exit"]
    main_menu_rects = []
    for i, opt_key in enumerate(menu_opts_keys):
        btn_y = 280 + i * 80 + offset_y
        btn_rect = pygame.Rect(640 - 100, btn_y, 200, 60)
        main_menu_rects.append((btn_rect, opt_key))
    ...
    # --- TEXTO VERSIÓN (Esquina inferior izquierda, semitransparente) ---
    version_surf = assets.ui_font_small.render(f"v{GAME_VERSION}", False, "white")
    version_surf.set_alpha(128)
    # RESTAMOS offset_y PARA QUE BAJE
    surface.blit(version_surf, (20, 720 - version_surf.get_height() - 20 - offset_y))

    # --- BOTÓN DE IDIOMA (Esquina inferior derecha) ---
    # RESTAMOS offset_y PARA QUE BAJE
    lang_rect = pygame.Rect(1280 - 70, 720 - 70 - offset_y, 50, 50)
    is_lang_pressed = lang_rect.collidepoint(mouseX, mouseY) and pygame.mouse.get_pressed()[0]
    draw_slot(surface, assets, lang_rect.x, lang_rect.y, 50, "", is_pressed=is_lang_pressed, show_ribbon=False, icon_img=getattr(assets, "lang_icon", None))

    main_menu_rects.append((lang_rect, "Language"))

    return main_menu_rects

def draw_language_menu(surface, assets, mouseX, mouseY, lang):
    overlay = pygame.Surface((1280, 720), pygame.SRCALPHA)
    overlay.fill((0, 0, 0, 200)) # Gris oscuro semitransparente como Castle Select
    surface.blit(overlay, (0, 0))

    title_str = lang.get("text_choose_language", "Choose language")
    title_text = assets.ui_font_large.render(title_str, False, "yellow")
    surface.blit(title_text, (640 - title_text.get_width() // 2, 200))

    lang_opts = [("English", "en", getattr(assets, 'en_icon', None)), ("Español", "es", getattr(assets, 'es_icon', None))]
    lang_rects = []

    for i, (name, code, icon) in enumerate(lang_opts):
        btn_y = 300 + i * 80
        btn_rect = pygame.Rect(640 - 120, btn_y, 240, 60)
        lang_rects.append((btn_rect, code))
        ...
        # Dibujar la banderita si existe
        if icon:
            scaled_icon = pygame.transform.scale(icon, (32, 24))
            surface.blit(scaled_icon, (btn_rect.x + 20, btn_y + 14 + y_off))

        opt_surf = assets.ui_font_medium.render(name, False, "black")
        surface.blit(opt_surf, (640 - opt_surf.get_width() // 2 + 15, btn_y + 30 - opt_surf.get_height() // 2 - 4 + y_off))
